...
def threes(counts):
    point_values = {1: 1000, 6: 600, 5: 500, 4: 400, 3: 300, 2: 200}
    for k, v in counts.items():
        if v >= 3:
            counts[k] -= 3
            return point_values[k]
    return 0


def singles(counts):
    point_values = {1: 100, 5: 50}
    points = 0
    for k, v in counts.items():
        points += v * point_values.get(k, 0)
    return points


if __name__ == "__main__":
    logging.info(score([5, 1, 3, 4, 1]))
    logging.info(score([1, 1, 1, 3, 1]))
    logging.info(score([2, 3, 4, 6, 2]))
    logging.info(score([4, 4, 4, 3, 3]))
    logging.info(score([2, 4, 4, 5, 4]))
