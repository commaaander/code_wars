...
def is_prime(number: int) -> bool:
    ...
    try:
        return primes_in_range[number]
    except KeyError:
        if number < 2:
            primes_in_range[number] = False
            return False
        if number <= 3:
            primes_in_range[number] = True
            return True
...
if __name__ == "__main__":
    ...
    # test.assert_equals(gap(100, 1000000, 2000000), [2, 3])
    # test.assert_equals(gap(2, 5, 7), [3, 5])
